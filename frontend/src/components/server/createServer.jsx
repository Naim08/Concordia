...
const CreateServerForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const server = {
      name,
      serverPhotoUrl: picture || null,
      ownerId: sessionUser.id,
    };

    const formData = new FormData();
    formData.append("server[name]", server.name);
    formData.append("server[ownerId]", server.ownerId);
    if (server.serverPhotoUrl) {
      formData.append("server[serverPhotoUrl]", server.serverPhotoUrl);
    }
